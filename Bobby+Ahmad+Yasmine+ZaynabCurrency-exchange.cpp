...
double getIsraelPounds(double americanDollars, double IS_POUND_RATE); 
...
const double IS_POUND_RATE = 3.78;
...
{
  cout << fixed << setprecision(2);
  cout << "$" << americanDollars << " US dollars in Israeli Pounds is ₪" << getIsraelPounds(americanDollars, IS_POUND_RATE);
}
else
{
  cout << fixed << setprecision(2);
...
double getIsraelPounds(double americanDollars, double IS_POUND_RATE)
{
  double convertedAmount = americanDollars * IS_POUND_RATE;
  return convertedAmount;
}
